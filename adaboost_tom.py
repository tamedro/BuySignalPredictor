...
def predict_volatility_1year_ahead(rows, day, num_days):
    ...
    f  = open('log', 'a')
    f.write('Day: ' + str(day) + '\n')
    f.write('Estimator weights: ' + str(regr.estimator_weights_) + '\n')
    f.write('Estimator errors: ' + str(regr.estimator_errors_) + '\n')
    f.close
...
log1 = []
#for ii in range(95):
    #print('iteration ' + str(ii) + ' of 95') 
    #predicted = predict_volatility_1year_ahead(rows, ii * 63 + 252, regression_days)    # 63 days is exactly 1/4 of a trading year
    #actual    = float(rows[ii*63][9])
    #log1.append("prediction for day:" + str(ii*63) + " predicted:" + str(predicted) + " actual:" + str(actual) + " error:" + str((predicted-actual)**2))


log2 = []
for ii in range(6250):
#for ii in range(1):
    print('iteration ' + str(ii) + ' of 12000') 
    predicted = predict_volatility_1year_ahead(rows, ii + 252, regression_days)    # 63 days is exactly 1/4 of a trading year
    actual    = float(rows[ii][9])
    log2.append(str(ii) + ',' + str(predicted-actual**2))
