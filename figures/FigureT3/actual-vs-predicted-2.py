...
class CustomClassifier(BaseEstimator, ClassifierMixin):
     ...
     def fit(self, X, y):
         ...
         # 2+ Adaboost iteration: use linera regreession as a weak learner
             ...
             self.regr = linear_model.Lasso(alpha=0.001,fit_intercept=False,normalize=False,max_iter=3)
             self.regr.fit(X, y)

     def predict(self, X):
         # 1st Adaboost iteration: just return the current volatility
         if self.instance <= 2:
             return X[:,2]   # return 6th element of feature vector (which is the current volatility) 
         # 2+ Adaboost iteration: use linera regreession as a weak learner    
         else:
